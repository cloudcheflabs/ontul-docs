...
- **SQL Parsing & Planning**: Parses SQL using Apache Calcite with a rule-based optimizer. Supports standard DML (INSERT, UPDATE, DELETE, MERGE INTO), DDL (CREATE TABLE, DROP, ALTER, CREATE VIEW), metadata commands (SHOW CATALOGS/SCHEMAS/TABLES, DESCRIBE, EXPLAIN), and SELECT queries with JOINs (INNER/LEFT/RIGHT/FULL), aggregations, grouping, CTEs and derived tables. The planned query then passes a capability gate that rejects constructs the execution engine cannot evaluate correctly — see [SQL Capability Gate](../features/sql-capability-gate.md).
