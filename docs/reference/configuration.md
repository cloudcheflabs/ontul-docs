...
| `ontul.iam.operation.permission.enforced` | `false` | Whether a missing `ontology:InvokeOperation` grant BLOCKS an ontology action that writes to an EXTERNAL system, or is only reported. The grant's resource namespace is `operation:<operationCatalog>:<operation>` — separate from `data:table:…` used for reads, so broad read access cannot imply the right to post into an ERP. Report-only by default so upgrading does not revoke working actions: the missing grant is logged and audited as `action:invoke:grant-missing` with the exact resource, then set this to `true` to enforce. Applies to both the REST invoke endpoint and SQL `CALL`. |
| `ontul.sql.strict` | `true` | SQL capability gate. The master validates every planned query and rejects constructs the execution engine cannot evaluate correctly (window functions, subqueries, `INTERSECT`/`EXCEPT`/de-duplicating `UNION`, `COUNT(DISTINCT)`, `OFFSET`, composite/non-equi join conditions, unimplemented scalar functions, …) with an explicit error. Set to `false` ONLY to triage a regression: it restores the previous behaviour where those queries run and return **wrong results**. `DELETE`/`UPDATE`/`MERGE` predicates are never gated. See [SQL Capability Gate](../features/sql-capability-gate.md). |
