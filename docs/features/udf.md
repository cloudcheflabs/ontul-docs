...
- **External libraries** — UDFs can use any third-party JAR. Dependency JARs are uploaded alongside the UDF class into `data/deps/` and loaded **child-first**: a JAR your UDF ships wins over the worker runtime's own copy of the same library, so you can bring your own version of common libraries (Jackson, Guava, …) without being shadowed by the runtime. Shared/boundary types stay runtime-loaded so identity is stable across the worker↔UDF boundary — `java.*`, `org.apache.arrow.*` (Arrow buffers cross the boundary), `com.cloudcheflabs.ontul.*` (the SDK's `MapFunction`/connector SPI), and `org.slf4j.*`. If you have your own shared library whose types cross the worker↔UDF boundary (e.g. a common UDF-interface JAR), add its package prefixes to `ontul.deps.parent.first.prefixes` (comma-separated) so it too stays runtime-loaded.
...
- **Auto-upload + cached child-first classloader** — `WorkerServer.buildDepsClassLoader()` builds a child-first `ChildFirstClassLoader` over `data/deps/`, caches it, and invalidates on directory change (file size + mtime fingerprint). Child-first delegation lets uploaded dependency JARs override the runtime's copies of common libraries (except the shared/boundary packages that must stay runtime-loaded); a plain parent-first classloader would let the runtime's versions win instead. UDFs that hold static models (caches, embeddings, ML weights) are loaded once per worker and reused across queries.
